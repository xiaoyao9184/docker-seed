...
function Start-SeedDocker() {
    ...
    if($local){
        ...
        $_playbook_file = Resolve-Path -Path "${_script_dir}/seed-remote.yml"
        $docker.volume.Add(@{
            source = $_playbook_file
            target = '/seed-remote.yml'
        }) | Out-Null
        ...
        $docker.command = "$_verbose --extra-vars 'user=$_remote_user host=$_remote_host' /seed-remote.yml"
    }
...
Set-Alias -Name "seed-docker" -Value Invoke-SeedDocker
